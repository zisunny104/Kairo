/**
 * 輔助標記草稿 API 路由
 * 把「輔助標記」分頁的工作進度存成伺服器上單一份共用草稿（runtime/assist-mark/draft.json），
...
function requireAdminToken(req, res, next) {
  ...
  if (!safeEqual(token, ADMIN_TOKEN)) {
    Logger.warn(`assist-mark 管理端點未授權存取 | ip=${req.ip} | path=${req.path}`);
...
const ASSIST_MARK_DIR = path.join(__dirname, "../../runtime/assist-mark");
const DRAFT_PATH = path.join(ASSIST_MARK_DIR, "draft.json");

async function ensureDir() {
  try {
    await fs.access(ASSIST_MARK_DIR);
  } catch {
    await fs.mkdir(ASSIST_MARK_DIR, { recursive: true });
  }
}

/**
 * GET /api/assist-mark/draft
 * 讀取伺服器上共用的輔助標記草稿；尚未存過時回傳 draft: null
 */
router.get("/draft", requireAdminToken, async (req, res) => {
  try {
    ...
    Logger.error("[AssistMark] Read error:", error.message);
    res.status(500).json({ success: false, error: "伺服器內部錯誤" });
  }
});

/**
 * PUT /api/assist-mark/draft
 * 整份覆寫伺服器上共用的輔助標記草稿
 */
router.put("/draft", requireAdminToken, async (req, res) => {
  try {
    ...
    Logger.error("[AssistMark] Save error:", error.message);
    res.status(500).json({ success: false, error: "伺服器內部錯誤" });
  }
});
